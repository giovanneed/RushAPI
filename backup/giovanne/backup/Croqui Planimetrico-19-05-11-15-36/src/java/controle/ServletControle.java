...
public class ServletControle extends HttpServlet
{
    ...
    throws ServletException, IOException
    {
        ...
        if (System.getProperty("os.name").indexOf("Windows")!=-1){
           output  = request.getSession().getServletContext().getRealPath("") + "\\" + session.getServletContext().getInitParameter("output_path") + "\\";
        }else{
            output = request.getSession().getServletContext().getRealPath("") + "/" + session.getServletContext().getInitParameter("output_path") + "/";
        }
        ...
        if (request.getParameter("function") != null)
        {
            ...
            if (request.getParameter("function").compareTo("blotes") == 0)
            {
                ...
                servicos.Servicos.limparDir(output + session.getId());
