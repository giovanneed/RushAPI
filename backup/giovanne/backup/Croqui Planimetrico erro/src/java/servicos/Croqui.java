...
public class Croqui
{
	...
	public BufferedImage gerarCroquiTipo1(com.lowagie.text.Rectangle p_Tamanho,
			...
			String p_NomeTabelaIptuGeoCtm) throws SQLException,
			CroquiException
			{
		...
		String v_ConsultaQuadra = "select g.geoloc " + GEOLOC;
		v_ConsultaQuadra += " from " + p_NomeTabelaQuadraCtm + " g";
		v_ConsultaQuadra += " where g.NUQDRCTM = " + p_PKQuadra;
		...
		String v_ConsultaLogradouro = getConsultaLogradouro(p_NomeTabelaTrecho);

		preencherLogradouro(v_BufferedImagem, v_Graficos, v_AlturaUtilizada, v_ConsultaLogradouro, v_DesvioParaCaberElementosAoRedor);
        ...
        String v_ConsultaLote = "select l.num_lote_ctm " + OBJECTID + ", concat(substr(ipt.indice_cadastral, 7, 4), concat(';',concat(substr(l.num_lote_ctm, -3, 3), concat(';', ipt.numero_imovel)))) " + VALOR + ", l.geoloc " + GEOLOC + ", sdo_geom.sdo_centroid(l.geoloc, .05) " + CENTROID + ",sdo_geom.sdo_area(l.geoloc,'5e-8','unit=sq_m') " + AREA;
		v_ConsultaLote += " from " + p_NomeTabelaLoteCtm + " l, " +  p_NomeTabelaQuadraCtm + " q, " + p_NomeTabelaIptuGeoCtm + " ipt ";
		v_ConsultaLote += " where q.NUQDRCTM = " + p_PKQuadra;
		v_ConsultaLote += " AND ipt.NULOTCTM = l.num_lote_ctm";
		v_ConsultaLote += " AND sdo_relate(l.geoloc, q.geoloc, 'mask=anyinteract') = 'TRUE'";
		...
		{
			v_ConsultaLote += " AND l.num_lote_ctm in(" + v_ConsultaDeLotes + ")";
		}
		...
		while(v_ResultSet.next())
		{
			if (p_PKsLotesADestacar.contains(v_ResultSet.getString(OBJECTID)))
			{
                ...
                String v_ConsultaEdificacao = "select e.geoloc " + GEOLOC + ",sdo_geom.sdo_centroid(e.geoloc, .05) " + CENTROID + ", sdo_geom.sdo_area(e.geoloc,'5e-8','unit=sq_m') " + AREA;
				v_ConsultaEdificacao += " from " + p_NomeTabelaLoteCtm + " l, " + p_NomeTabelaEdificacao + " e ";
				v_ConsultaEdificacao += " where l.num_lote_ctm = " + v_ResultSet.getString(OBJECTID);
				v_ConsultaEdificacao += " AND (SDO_GEOM.RELATE(SDO_GEOM.SDO_CENTROID(E.geoloc,.5), 'DETERMINE',l.geoloc, 0.5) = 'INSIDE' ";
				v_ConsultaEdificacao += " OR SDO_GEOM.RELATE(SDO_GEOM.SDO_CENTROID(E.geoloc,.5), 'DETERMINE',l.geoloc, 0.5) = 'COVEREDBY') ";
				v_ConsultaEdificacao += "AND sdo_relate(e.geoloc, l.geoloc, 'mask=anyinteract') = 'TRUE' ";
		...
		String v_ConsultaLoteNaoDestacar = "select l.num_lote_ctm " + OBJECTID + " , l.geoloc " + GEOLOC;
		v_ConsultaLoteNaoDestacar += " from " + p_NomeTabelaLoteCtm + " l, " +  p_NomeTabelaQuadraCtm + " q";
		v_ConsultaLoteNaoDestacar += " where q.NUQDRCTM = " + p_PKQuadra;
		v_ConsultaLoteNaoDestacar += " AND sdo_relate(l.geoloc, q.geoloc, 'mask=anyinteract') = 'TRUE'";

		if (!v_ConsultaDeLotes.equals(""))
		{
			v_ConsultaLoteNaoDestacar += " AND l.num_lote_ctm not in(" + v_ConsultaDeLotes + ")";
		}
		...
		escreverCabecalhoTipo1(v_BufferedImagem, v_Graficos, m_CorQuadra, m_CorLote, m_CorEdificacao, m_CorLoteIptu, m_CorLoteCtm, m_CorLoteEndereco, p_PKQuadra, p_NomeTabelaQuadraCtm, p_NomeTabelaEdificacao, p_NomeTabelaBairroPopular, p_NomeTabelaIptuGeoCtm);
	...
	public BufferedImage gerarCroquiTipo2(com.lowagie.text.Rectangle p_Tamanho,
			...
			String p_NomeTabelaEdificacao, String p_NomeTabelaIptuCtmGeo) throws SQLException
			{
		limparCampos();
		...
		int v_AlturaUtilizada = escreverCabecalhoTipo2(v_BufferedImagem, v_Graficos, m_CorLote, m_CorEdificacao, p_PKLote, p_NomeTabelaIptuCtmGeo);
        v_AlturaUtilizada += p_MargemPixel;
		/*String v_NomeTabelaLoteCtm = p_OwnerTabelaLoteCtm + "." + LOTE_CTM;
		String v_NomeTabelaEdifParticular = p_OwnerTabelaEdificacao + "." + EDIFICACAO;*/

		String v_Consulta = getConsultaLote(p_NomeTabelaLoteCtm, p_PKLote);
		...
		v_Consulta = getConsultaLote(p_NomeTabelaLoteCtm, p_PKLote);
		preencherGeometria(v_BufferedImagem, v_Graficos, v_AlturaUtilizada, null, m_CorLote, v_CorCoordenadasSegmentoLote, v_CorDivisaoSegmentosEdificacao, v_CorCentroidLote, null, v_Consulta, (String) p_NomeTabelaLoteCtm.subSequence(p_NomeTabelaLoteCtm.indexOf(".") + 1, p_NomeTabelaLoteCtm.length()), v_DesvioParaCaberElementosAoRedor);
	...
	public BufferedImage gerarCroquiTipo3(com.lowagie.text.Rectangle p_Tamanho,
			String p_PKLote, int p_MargemPixel,
			String p_NomeTabelaLoteCtm,
			String p_NomeTabelaIptuCtmGeo) throws SQLException
			{
		limparCampos();
		...
		double v_DesvioParaCaberElementosAoRedor = 1;
		int v_AlturaUtilizada = escreverCabecalhoTipo3(v_BufferedImagem, v_Graficos, m_CorLote, p_PKLote, p_NomeTabelaIptuCtmGeo);
	...
	private void escreverCabecalhoTipo1(BufferedImage p_BufferedImage, Graphics2D p_Graficos,
		...
		String v_NomeBairro = getNomeBairroDaQuadra(p_PKQuadra, p_NomeTabelaQuadra, p_NomeTabelaBairroPopular);
		String v_IndiceCadastral = getIndiceCadastralDaQuadra(p_PKQuadra, p_NomeTabelaIptuGeoCtm);
	...
	private int escreverCabecalhoTipo2(BufferedImage p_BufferedImage, Graphics2D p_Graficos,
			Color p_CorLote, Color p_CorEdificacao, String p_PKLote, String p_NomeTabelaIptuCtmGeo) throws SQLException
			{
		Font v_FonteOriginal = p_Graficos.getFont();
	...
	private int escreverCabecalhoTipo3(BufferedImage p_BufferedImage, Graphics2D p_Graficos,
			Color p_CorLote, String p_PKLote, String p_NomeTabelaIptuCtmGeo) throws SQLException
			{
		String v_CodigoIptu = getCodigoIptu(p_PKLote, p_NomeTabelaIptuCtmGeo);
	 ...
	 * @param p_OwnerTabelaIptuGeoCtm Indica o owner da tabela de iptu
	 */
	private String getCodigoIptu(String p_PKLote, String p_NomeTabelaIptuCtmGeo) throws SQLException
	{
		Statement v_Statement = m_Conexao.createStatement();
...
//		String v_Consulta = "select substr(indice_cadastral, 1, 10) from " + v_NomeIptuGeomCtm;
		String v_Consulta = "select indice_cadastral from " + p_NomeTabelaIptuCtmGeo;
		v_Consulta += " where nulotctm = " + p_PKLote;
		ResultSet v_ResultSet = v_Statement.executeQuery(v_Consulta);
	...
	private String getNomeBairroDaQuadra(int p_PKQuadra, String p_NomeTabelaQuadra,
			String p_NomeTabelaBairroPopular) throws SQLException
			{
		...
		v_Consulta += " where q.nuqdrctm = " + p_PKQuadra;
		v_Consulta += " AND sdo_relate(q.geoloc, b.geometria, 'mask=anyinteract') = 'TRUE'";
	...
	private String getIndiceCadastralDaQuadra(int p_PKQuadra,
			String p_NomeTabelaIptuCtmGeo) throws SQLException, IndiceCadastralNaoEncontradaParaAQuadraCroquiException
			{
		/*String v_NomeTabelaIptuGeoCtm = p_OwnerTabelaIptuCtmGeo + "." + IPTU_CTM_GEO;*/
        String v_NomeCampoIndiceCadastral = "indice";

        String v_Consulta = "select i.indice_cadastral " + v_NomeCampoIndiceCadastral;
        v_Consulta += " from "+p_NomeTabelaIptuCtmGeo+ " i ";
        v_Consulta += " where i.nulotctm like '"+p_PKQuadra+"%' ";
        v_Consulta += " and rownum = 1 ";
		...
		{
			throw new IndiceCadastralNaoEncontradaParaAQuadraCroquiException("Indice Cadastral n�o encontrado para a quadra");
		}
	...
	private void preencherLogradouro(BufferedImage p_BufferedImagem, Graphics2D p_Graficos, int p_AlturaUtilizada,
			String p_Consulta, double p_DesvioParaCaberElementosAoRedor) throws SQLException
			{
		...
		while(v_ResultSet.next())
		{
            ...
            String v_NovaConsulta = p_Consulta + " AND t.mi_prinx = " + v_ObjectId;
            ...
            String v_Consulta = "select  id_logradouro " + OBJECTID + ",";
			v_Consulta += "sdo_cs.transform(SDO_LRS.CONVERT_TO_STD_GEOM(SDO_LRS.LOCATE_PT(SDO_LRS.CONVERT_TO_LRS_GEOM(GEOLOC, 3), SDO_GEOM.SDO_LENGTH(GEOLOC, 3) / 2)), 82301) " + PONTO + " from trecho";
			v_Consulta += "  where mi_prinx = " + v_ObjectId;
...
// mario - aqui escreve o id da linha
	 ...
	 */
	private String getConsultaLogradouro(String p_NomeTabelaTrecho)
	{
		String v_ConsultaLogradouro = "select t.mi_prinx " + OBJECTID + ", concat(t.id_logradouro, concat(' ', concat(t.tplog, concat(' ', t.nolog)))) " + NULOG + ", t.geoloc " + GEOLOC + " from " + p_NomeTabelaTrecho + " t";
		v_ConsultaLogradouro += " WHERE SDO_FILTER(t.GEOLOC, mdsys.sdo_geometry(2003,82301,NULL,";
	...
	/**
	 * Procedimento que defini as menores e maiores coordenadas X e Y
	 * @return Retorna a consulta padr�o de busca de lote
	...
	private String getConsultaLote(String p_NomeTabelaLoteCtm, String p_PKLote)
	{
		...
		String v_ConsultaLote = "select l.num_lote_ctm " + OBJECTID + ",  l.geoloc " + GEOLOC + ",  sdo_geom.sdo_centroid(l.geoloc, .05) " + CENTROID + ", sdo_geom.sdo_area(l.geoloc,'5e-8','unit=sq_m') " + AREA;
		v_ConsultaLote += " from " + p_NomeTabelaLoteCtm + " l ";
		v_ConsultaLote += " where l.num_lote_ctm = " + p_PKLote;
