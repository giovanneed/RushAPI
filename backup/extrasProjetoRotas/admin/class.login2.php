...
class logmein {
	...
	var $database_logon = 'rotasdacopa';		//Database NAME
	...
	var $pass_column = 'senha';		//PASSWORD column
	var $user_level = 'nivel';		//(optional) userlevel column
	...
	function loginform($formname, $formclass, $formaction){
		...
		echo'<form name="'.$formname.'" method="post" id="'.$formname.'" class="'.$formclass.'" enctype="application/x-www-form-urlencoded" action="'.$formaction.'">
		        <center>
				<table border=1>
				<tr><td colspan="2" align="center"><b>Cadastro POI</b></td></tr>
				<tr><td align="right"><label for="username">Usu&aacute;rio:</label></td><td><input name="username" id="username" type="text"></td></tr>
				<tr><td align="right"><label for="password">Senha:</label></td><td><input name="password" id="password" type="password"></td></tr>
				<tr><td colspan="2" align="center"><input name="action" id="action" value="login" type="hidden">
				<input name="submit" id="submit" value="Login" type="submit"></td></tr>
				</table>
				</center>
			</form>';
	}
...
function register($table, $username, $password, $nivel){ 
    //conect to DB
    $this->dbconnect(); 
    //make sure table name is set 
    if($this->user_table == ""){ 
    ...
    $result = $this->qry("INSERT INTO ".$this->user_table." VALUES(DEFAULT,'?','?',?)", $username, $password, $nivel); 
    $row=mysql_fetch_assoc($result); 
    if($row != "Error"){ 
    if($row[$this->user_column] !="" && $row[$this->pass_column] !=""){ 
        //register sessions 
