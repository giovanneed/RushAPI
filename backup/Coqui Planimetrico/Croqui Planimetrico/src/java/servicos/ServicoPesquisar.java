            ...
            String table_iptu_ctm_numero_do_lote,String table_iptu_ctm_geo_numero_do_imovel)
    {
        ...
        if (loteIptu.length() > 0)
        {
            if (loteIptu.length() == 6)
                loteIptu += " ";
            loteIptu += '%';
            return dao.ServicoDao.consultaPorIptu(loteIptu, pc, table_edificacao, table_lote_ctm, table_iptu_ctm_geo, geometria_edificacao, geometria_lote_ctm, numero_do_lote_ctm, iptu_ctm_geo_indice_cadastral, iptu_ctm_geo_area_construida, table_iptu_ctm_numero_do_lote);
        }
        else if (lote.length() > 0){
            lote += '%';
            return dao.ServicoDao.consultaPorLote(lote, pc, table_edificacao, table_lote_ctm, table_iptu_ctm_geo,geometria_edificacao,geometria_lote_ctm,numero_do_lote_ctm,iptu_ctm_geo_indice_cadastral,iptu_ctm_geo_area_construida,
